...
| **合计（全做）** | **~11~14KB** | +0.5KB | ~600B | | 17KB 预算内偏紧 |

**结论**：深度优化后 17.0KB 预算，推荐清单全做可行（剩 ~3-6KB）。分批：第一批 NTC+计算器+闹钟（~4.1~5.5KB），第二批**红外遥控**+贪吃蛇/2048（~4~5KB），第三批串口+数码管（~2.5~3.5KB）。每个功能完成后构建一次监控体积。
