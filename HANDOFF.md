...
## 4. 内存预算与功能规划（2026-08-16 评估）

**当前占用（构建 a1281b3）**：Flash 52.9KB/64KB（剩 **11.1KB**）、xdata 3861B/8KB（剩 4.2KB）、data 15.1B、0 Error/6 Warning

**本次会话动作**：
- v0.3 tag 已打并推送；config.h 恢复 VIRTUAL_OLED=0（实体屏），工作区干净
- **删除 9 个零引用图标回收 Flash 3.4KB**（提交 a1281b3）：Image_settings_64/calc_64/wechat_64/alipay_64/night_64/more_64（各 512B）+ Image_window/sleep（各 128B）+ Image_cube——均零引用，C51 不移除未用 const，白占。**教训：新增图标前 grep 引用，零引用 const 立即删**
- 图标盘点方法：`grep -rl --include="*.c" --include="*.h" "\bImage_x\b" src/ | grep -v OLED_Fonts` 判引用；删除用 python GBK 处理（勿用 Edit 工具）
...
| 串口通信 | 1.5~2.5KB | +0.26KB（环形缓冲 256B） | ~160B（波特率/接收） | ★★☆ DB9 座直插（见下） | 第二批（可砍） |
| **合计** | **7.1~10.2KB** | +0.5KB | ~500B | | 全做可行（剩 1~4KB） |

**结论**：回收前 6.7KB 必须五选三；回收后 11.1KB **五个全做可行但贴近上限**。推荐分批：第一批 NTC+计算器+闹钟（~4.1~5.5KB，即插即用演示），第二批贪吃蛇+串口（~3~4.7KB，装完剩 ~1.5KB）。每个功能完成后构建一次监控体积，超预算先砍串口。
