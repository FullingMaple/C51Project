...
**当前占用（构建 2026-08-16 优化后）**：Flash **46.9KB/64KB（剩 17.1KB）**、xdata 3861B/8KB（剩 4.2KB）、data 15.1B、0 Error/6 Warning
...
2. **字库深度裁剪 -6.1KB**（本次）：按"代码字符串字面量 → 渲染尺寸分组"分析，只保留真实引用的字——
   ...
   - 12×12：122 → **31 字**（列表/窗口文本全集）
   - 8×8：53 → **1 字占位**（FONT_8 无页面使用，留 1 字防链接错误）
   - **存量 bug 修复**：12×12 原库缺"协议"（关于页"协议: Apache 2.0"一直显示空白）、补 2 字（协/议）——"表"原库已有
