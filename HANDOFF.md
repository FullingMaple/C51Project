...
| **秒表**（2026-08-16 新增） | ~0.8KB | ~0 | ~80B（计次/圈数） | ★★☆ 手表功能三件套 | 第一批 |
| **红外遥控**（控菜单+按键音） | ~2.5KB | ~0 | ~130B（遥控/对码等） | ★★★ 遥控全场操作 | 第二批 |
| **2048**（2026-08-16 新增） | ~2.0KB | ~0.5KB（4×4 棋盘） | ~50B（得分） | ★★★ 互动 | 第二批 |
| 串口通信 | 1.5~2.5KB | +0.26KB（环形缓冲 256B） | ~160B（波特率/接收） | ★★☆ DB9 座直插（见下） | 第三批（可砍） |
| 数码管时钟联动 | ~1KB | ~0 | 0 | ★★☆ 双屏 | 第三批 |
| **合计（全做）** | **~15~18KB** | +1KB | ~700B | | **略超 17KB 预算！需取舍** |

**结论**：全做 ~15-18KB 超出 17KB 预算——**必须取舍**。推荐取舍（省 ~4KB）：
1. **贪吃蛇取消**（与 2048 二选一，保留 2048——方向键契合遥控、界面整齐）
2. **串口砍掉**（演示需带 DB9 转接线，性价比最低）
→ 精简后：NTC+计算器+闹钟+时钟+秒表+遥控+2048+数码管 ≈ **13~14KB**（剩 ~3KB 余量）
分批：第一批 NTC→计算器→闹钟→时钟/万年历→秒表；第二批遥控→2048；第三批数码管联动（串口看情况）。每个功能完成后构建一次监控体积。
