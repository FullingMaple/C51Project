...
void OLED_UI_ShowClock(void)
{
    ...
    if(CurrentMenuPage != &MainMenuPage) return;   /* 仅磁贴主菜单页显示（其余页面不再全局显示） */
    RTC_GetTime(&t);
...
void SetTargetScrollBarHeight(void){
	MenuID num;   /* 菜单项数（0 菜单项页面防除零） */
	num = GetMenuItemNum(CurrentMenuPage->General_MenuItems);
	if(num == 0) return;   /* 万年历/时间设置等 0 菜单项页面：无滚动条 */
	if(CurrentMenuPage->General_MenuType == MENU_TYPE_LIST){
...
MenuID_Type OLED_KeyAndEncoderRecord(void){
	...
	//（0 菜单项页面 MaxID==0：MaxID-1 == -1 会让 ID 回绕成 -1，必须保持 0）
	if(MaxID == 0){
		ActiveMenuID = 0;
	}else{
